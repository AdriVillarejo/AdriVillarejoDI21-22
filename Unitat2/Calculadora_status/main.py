...
from PySide6.QtWidgets import (
    QApplication, QLineEdit, QMainWindow, QVBoxLayout, QWidget, QGridLayout, QPushButton,
     QStackedLayout, QLabel, QStatusBar, QMessageBox,
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.boton_salir = QAction("&Salir", self)
        self.boton_salir.triggered.connect(self.salir)
        self.boton_salir.setShortcut(QKeySequence("Ctrl+s"))
        ...
        file_menu.addAction(self.boton_salir)
        ...
        # Definimos la barra de estado
        self.barra_estado = QStatusBar()
        self.setStatusBar(self.barra_estado)
        self.barra_estado.showMessage("Calculadora Estandar")
        self.label_guardado = QLabel('Guardado desactivado')
        self.barra_estado.addPermanentWidget(self.label_guardado)


        if operacions.isChecked:
            self.label_guardado.setText("Guardando ")
        elif not operacions.isChecked:
            self.label_guardado.setText(" ")


        # Ponemos que en la status bar salga por defecto "Calculadora normal"
        self.barra_estado.showMessage("Calculadora Normal :)")
        ...
        for button, posicion in letras_calculadora.items():
            ...
            self.letras[button].setStatusTip(button)

            if (button == '<-'):
                self.letras[button].setStatusTip( ( "Back ") )
            elif (button == '('):
                self.letras[button].setStatusTip( ( " Shift + 8 ") )
            elif (button == ')'):
                self.letras[button].setStatusTip( ( " Shift + 9 ") )
            elif (button == '%'):
                self.letras[button].setStatusTip( ( " Shift + 5 ") )
        ...
        for button_cien, posicion_cien in letras_calculadoracientifica.items():
            ...
            if (button == '<-'):
                self.letras_cien[button_cien].setStatusTip( ( "Back ") )
            elif (button == '('):
                self.letras_cien[button_cien].setStatusTip( ( " Shift + 8 ") )
            elif (button == ')'):
                self.letras_cien[button_cien].setStatusTip( ( " Shift + 9 ") )
            elif (button == '%'):
                self.letras_cien[button_cien].setStatusTip( ( " Shift + 5 ") )
    ...
    # Esta función es para que nos devuelva Back al poner el ratón sobre la flecha de atrás (pero no funciona)
    def back(self):
        self.barra_estado.showMessage("Backspace")
    ...
    def cientifica(self):
        ...
        self.barra_estado.showMessage("Calculadora Cientifica")
    ...
    def salir(self):
        ...
        salir_boton.setText("Quieres salir? ")
